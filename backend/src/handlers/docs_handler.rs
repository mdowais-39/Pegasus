...
const ENDPOINTS: &[(&str, &str, &str, &str)] = &[
    ...
    ("delete", "/api/v1/statements/{id}", "ingestion",
        "Delete a statement and all its derived data (cascade)"),
    ("post", "/api/v1/database/clear", "ingestion",
        "Clear all statements and reset the investigation workspace"),
    ...
    ("get", "/api/v1/alerts", "alerts", "List investigator alerts (?unacknowledged=&limit=)"),
    ("get", "/api/v1/alerts/count", "alerts", "Unacknowledged alert count (nav bell)"),
    ("post", "/api/v1/alerts/{id}/ack", "alerts", "Acknowledge an alert"),
    ...
    ("get", "/api/v1/reports/{case_id}/service/{service}/{fmt}", "reporting",
        "Per-service report (round-trips|money-flow|money-trail) as json/pdf/excel/docx"),
];
