...
import { GitFork, Loader2, AlertTriangle, HelpCircle, Copy, Calendar, ArrowRight, ArrowLeft } from 'lucide-react';
          ...
          <div className="lg:col-span-8 flex flex-col space-y-3">
            ...
            <div className="flex items-center justify-between bg-white border border-[#E4E4E7] rounded-xl px-4 py-2.5 shadow-xs">
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] uppercase font-bold text-[#71717A] font-mono">View Mode:</span>
                <select
                  value={showAllNodes ? 'all' : 'top'}
                  onChange={(e) => setShowAllNodes(e.target.value === 'all')}
                  className="bg-[#FAF9F6] border border-[#E4E4E7] rounded text-[10px] font-semibold focus:ring-0 cursor-pointer font-sans text-zinc-950 p-1 px-2 focus:border-zinc-400"
                >
                  <option value="top">Top 15 Nodes (Clean)</option>
                  {totalNodesCount > 15 || showAllNodes ? (
                    <option value="all">Full Network ({totalNodesCount} nodes)</option>
                  ) : null}
                </select>
              </div>
            </div>
